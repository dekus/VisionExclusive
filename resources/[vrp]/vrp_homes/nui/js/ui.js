...
var templateTrunkChest = (key, amount, image, name, weight, target) => {
  var buttonText = target == 'inventory' ? 'Depositare' : 'Ritirare'; 
  ...
  <div class="cell" data-item-key="${key}">
    <div class="amount-option">
      ...
      <div class="row">
        <button class="button cancel" data-event="cancel">Annulla</button>
      </div>
    </div>
    <div class="row">
    ...
    <div class="row">
      <div class="image" style="background-image: url('https://images.visionexclusive.it/images/${image}.png');"></div>
    </div>
    <div class="row">
...
const updateVault = () => {
  ...
  $.post("http://vrp_homes/requestVault", JSON.stringify({}), data => {
    ...
    $('.bag-workspace > .inventory-title').html(`
      Inventario <small>(<i><b>${data.peso.toFixed(2)}kg</b> in uso - <b>${(data.maxpeso - data.peso).toFixed(2)}kg</b> liberi <b>${data.maxpeso.toFixed(2)}kg</b></i> totali)</small>
    `);
    $('.trunk-workspace > .inventory-title').html(`
      Baule da casa <small>(<i><b>${data.peso2.toFixed(2)}kg</b> in uso - <b>${(data.maxpeso2 - data.peso2).toFixed(2)}kg </b> livre de <b>${data.maxpeso2.toFixed(2)}kg</b></i> totali)</small>
    `);
