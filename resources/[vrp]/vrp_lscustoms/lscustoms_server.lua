...
function emP.checkPermission()
	...
	if user_id then
		local bennys = vRP.getUsersByPermission("Mecanico")
		if parseInt(#bennys) == 0 then
		return true
		else
		return (vRP.hasPermission(user_id,"Mecanico") or vRP.hasPermission(user_id,"Admin") or vRP.hasPermission(user_id,"Diamond"))
		end
	end
end

function emP.checkAdmin()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id,"Admin") or vRP.hasPermission(user_id,"Diamond") or vRP.hasPermission(user_id,"Investidor2") then
			TriggerClientEvent('Notify', source, "successo", "Hai effettuato l'accesso a Benny's come amministratore.")
			return true
		end
	end
end
	...
	if user_id then
		...
		if vehicle and placa then
			...
			if puser_id then
				...
				custom.bloccomotore = mods[39].mod
