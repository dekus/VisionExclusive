...
vRP.prepare("rusher/start_sel_userid","SELECT * FROM vrp_user_ids WHERE identifier = @licenca")
...
Config.Language = {
	joining = "Verifica delle informazioni in corso...",
	connecting = "Ti stiamo assegnando al server, attendi...",
	err = "Impossibile identificare il tuo Steam o Social Club.",
	_err = "Sei stato disconnesso per aver impiegato troppo tempo a fare la coda.",
	pos = "Sei il %d/%d della coda, aspetta la tua connessione. Visita il nostro negozio per la priorità: https://five-m.store/store",
	connectingerr = "Impossibile aggiungerti alla coda.",
	wlonly = "Non hai la whitelist entra su https://discord.gg/visionexclusive.",
	banned = "Sei bannato da questo server, per maggiori informazioni entra su https://discord.gg/visionexclusive.",
	steam = "Steam disconnesso, impossibile connettersi."
}
	...
	for k,v in ipairs(self.QueueList) do
		if tmp.priority then
			if not v.priority then
				print( '[CODA] ' .. (name or 'N/A' ) .. ' in coda in posizione' .. k .. ' (PRIORITÀ: ' .. tostring(tmp.priority) .. ')')
				_pos = k
			else
...
Citizen.CreateThread(function()
	local function playerConnect(name,setKickReason,deferrals)
		...
		local reason = "Sei stato espulso dall'unirti alla coda"
		...
		Citizen.CreateThread(function()
			...
			while true do
				...
				local msg = string_format("Vision Exclusive\n\n"..Config.Language.pos.."%s\nEvita le punizioni, rispetta le regole di condotta.\nAggiornamenti frequenti, lascia il tuo suggerimento nel nostro discord.\nVisita il nostro negozio per ottenere priorità nella coda e altri vantaggi per aiutare il server",pos,Queue:GetSize(),dots)
...
RegisterCommand('fila',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,'admin.autorizzazione') or vRP.hasPermission(user_id,'mod.autorizzazione') or vRP.hasPermission(user_id,'support.autorizzazione') then
		TriggerClientEvent("Notify",source,"avviso",(#Queue.QueueList).." persone in coda.")
	end
end)