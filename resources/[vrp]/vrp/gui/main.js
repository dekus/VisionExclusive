window.addEventListener("load",function(){
	...
	wpromptNum.onClose = function () {
		const res = parseInt(wpromptNum.result)
		$.post("https://vrp/promptNum", JSON.stringify({ act: "close", result: (isNaN(res)  ? false : res )  }));
	};
	$.post("https://vrp/init","");
	...
	window.addEventListener("message",function(evt){
		...
		if(data.act == "showredot") {
			var html = "<div id='img-container'><img src='https://cdn.discordapp.com/attachments/890793125889605732/1047479298166104095/icon.png'>"
			$(html).fadeIn(0).appendTo("#img-container").delay(170).fadeOut(0);  
		}
		else if(data.act == "cfg"){
			cfg = data.cfg
		}
		...
		} else if (data.act === 'promptNum'){
			wpromptNum.open(data.title, data.text);
		}
