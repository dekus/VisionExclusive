...
--[[ function vRP.removeUserGroup(user_id,group)
	local user_groups = vRP.getUserGroups(user_id)
	local groupdef = groups[group] -- GroupName

	if not groupdef then
		print('^1[ - ] VISION GROUPS >^7 Foi constatado um erro, o grupo digitado não existe.')
		return 
	end
	...
	local source = vRP.getUserSource(user_id)
	if source == nil then
		TriggerEvent("rusher:forceDbUpdate",user_id,group)
	end
end ]]
...
function vRP.removeUserGroup(user_id,group)
	...
	local gtype = nil
	if groupdef == nil then return; end
	if groupdef._config then
		gtype = groupdef._config.gtype 
	end
