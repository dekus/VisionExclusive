...
function openTattooShop()
    ...
    elseif GetEntityModel(ped) == GetHashKey("mp_f_freemode_01") then
        ...
        SetPedComponentVariation(ped, 1, -1, 0, 2)
        SetPedComponentVariation(ped, 3, 15, 0, 2)
        SetPedComponentVariation(ped, 4, 17, 9, 2)
        SetPedComponentVariation(ped, 5, -1, 0, 2)
    ...
    TaskPlayAnim(PlayerPedId(), "move_f@multiplayer", "idle", 1.0, -1, -1, 3, 0, false, false, false)
...
Citizen.CreateThread(function()
    local innerTable = {}
    for k,v in pairs(coordsSystem) do
        table.insert(innerTable,{ v[1],v[2],v[3],2.5,"E","Negozio di tatuaggi","Apri","Premi" })
    end
...
RegisterCommand("tattoos", function()
    local ped = PlayerPedId()
    if not IsPedInAnyVehicle(ped) and vSERVER.checkPermission() then
        customCamLocation = nil
        openTattooShop()
    end
end)
