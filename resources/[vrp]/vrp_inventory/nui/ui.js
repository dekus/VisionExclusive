...
var updateMochila = () => {
  ...
  $.post("http://vrp_inventory/requestMochila", JSON.stringify({}), data => {
    ...
    $('.inventory-title').html(`
      Inventario <small>(<i><b>${data.weight.toFixed(2)}kg</b> in uso | <b>${(data.maxWeight - data.weight).toFixed(2)}kg</b> liberi <b>${data.maxWeight.toFixed(2)}kg</b> in totale</i>)</small>
    `);
    $(".row.objects").html(`
    ${nameList
      .map(
        ...
        <div class="options">
          <div class="option" data-event="use">Usa</div>
          <div class="option" data-event="send">Inviare</div>
          <div class="option" data-event="drop">Buttare</div>
        </div>
        <div class="amount-option">
          ...
          <div class="row">
            <button class="button" data-event="send">Inviare</button>
          </div>
          <div class="row">
            <button class="button cancel" data-event="cancel">Annullare</button>
          </div>
        </div>
        <div class="row">
        ...
        <div class="row">
          <div class="image" style="background-image: url(http://images.visionexclusive.it/images/${item.index}.png)"></div>
        </div>
        <div class="row">
          <div class="name">${item.name} <i>(${(item.weight * item.amount).toFixed(2)}kg)</i></div>
        </div>
      </div>
    `
      )
