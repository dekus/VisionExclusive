				...
				if hoverLocates[gridZone] then
					for k,v in pairs(hoverLocates[gridZone]) do
						...
						if distance < v["distance"] then
							SendNUIMessage({ show = true, key = v["key"], title = v["title"], action = v["legend"], prefix = v["prefix"] })
...
AddEventHandler("hoverfy:insertTable",function(innerTable)
	for k,v in pairs(innerTable) do
		...
		if v[8] then prefix = v[8] else prefix = "Premi" end

		hoverNumbers = hoverNumbers + 1
		hoverLocates[gridZone][hoverNumbers] = { x = v[1], y = v[2], z = v[3], distance = v[4], key = v[5], title = v[6], legend = v[7], prefix = prefix }
	end
end)