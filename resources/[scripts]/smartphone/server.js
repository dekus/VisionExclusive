...
function pa() {
    ...
    console.log('Smartphone:: DRIVER DEL DATABASE INCOMPATIBILE');
    return console.log('Smartphone:: LO SCRIPT PRESUMERÀ CHE UTILIZZATE GHMattiMySQL'), 'GHMattiMySQL';
}
...
const ps = {
    ...
    'Passaporto_NOT_FOUND': 'Passaporto non trovato',
    'PROFILE_NOT_FOUND': 'Profilo non trovato',
    'PHONE_NOT_FOUND': 'Numero non trovato',
    'MESSAGE_BLOCKED': 'Non puoi inviare un messaggio a questo numero.',
    'PLAYER_OFFLINE': 'Abitante fuori città',
    'NO_PERMISSION': 'Senza permesso',
    'MESSAGE_TOO_LONG': 'Messaggio troppo grande',
    'USER.NO_ID': 'Impossibile recuperare il passaporto',
    'USER.NO_IDENTITY': 'Impossibile recuperare la tua identità',
    'TRANSFER.LOCK': 'Attendi il tuo trasferimento precedente',
    'TRANSFER.SELF': 'Non puoi trasferire a te stesso',
    'TRANSFER.NO_FUNDS': 'Saldo insufficente',
    'BANK.PIX_NOT_FOUND': 'Chave pix non trovata',
    'BANK.PIX_DISABLED': 'Il pix è disabilitato',
    'BANK.INVOICE_NOT_FOUND': 'Fattura non trovata',
    'BANK.INVOICE_SELF': 'Non puoi auto-farti una fattura',
    'BANK.INVOICE_ALREADY_PAID': 'Questa fattura è già stata pagata',
    'BANK.INVOICE_NOT_YOURS': 'Questa fattura non è tua',
    'BANK.INVOICE_REQUEST': 'Vuoi accettare la fattura {reason} di {name} con un valore di {value}?',
    'BANK.INVOICE_NOTIFY_TITLE': 'Fatture',
    'BANK.INVOICE_NOTIFY_ACCEPTED': '{name} ha accettato la tua fattura',
    'BANK.INVOICE_NOTIFY_REJECTED': '{name} ha rifiutato la tua fattura',
    'CALL.NOT_FOUND': 'Questa chiamata non esiste più',
    'CALL.OFFLINE': 'Numero fuori area',
    'CALL.LOCK': 'Sei già impegnato in una chiamata',
    'CALL.OCCUPIED': 'Questa linea è occupata',
    'INSTAGRAM.POST_NOT_FOUND': 'Questo post non è stato trovato',
    'INSTAGRAM.INVALID_USERNAME': 'Utente non valido, usa lettere/numeri con un massimo di 24 caratteri',
    'INSTAGRAM.INVALID_NAME': 'Nome non valido',
    'INSTAGRAM.ALREADY_REGISTERED': 'Hai già un account',
    'INSTAGRAM.USERNAME_TAKEN': 'Questo nome utente esiste già',
    'INSTAGRAM.LIMIT_REACHED': 'Hai raggiunto il massimo di account',
    'INSTAGRAM.REOPEN': 'Riapri l\'app (comunica con il comune)',
    'INSTAGRAM.REPLY': '{name} ha commentato il tuo post',
    'INSTAGRAM.MENTION': '{name} ti ha menzionato in un post',
    'INSTAGRAM.PUBLISH': '{name} ha pubblicato una foto',
    'INSTAGRAM.LIKE': '{name} ha apprezzato il tuo post',
    'INSTAGRAM.FOLLOW': '{name} ti sta seguendo',
    'INSTAGRAM.WAIT_USERNAME_CHANGE': 'Aspetta un\'ora per cambiare nuovamente il nome utente',
    'OLX.INVALID_TITLE': 'Titolo non valido',
    'OLX.CATEGORY_MANDATORY': 'La categoria è obbligatoria',
    'OLX.DESCRIPTION_MANDATORY': 'La descrizione è obbligatoria',
    'OLX.IMAGE_MANDATORY': 'L\'immagine è obbligatoria',
    'OLX.IMAGE_MAXIMUM': 'Il massimo di immagini è 3',
    'SERVICE.UNAVAILABLE': 'Servizio non disponibile al momento',
    'SERVICE.NOT_FOUND': 'Questo servizio non esiste',
    'SERVICE.ALREADY_SOLVED': 'Questa richiesta è già stata risolta',
    'TWITTER.INVALID_FORM': 'Modulo non valido',
    'TWITTER.INVALID_USERNAME': 'Utente non valido',
    'TWITTER.INVALID_NAME': 'Nome non valido',
    'TWITTER.INVALID_BIO': 'Biografia non valida',
    'TWITTER.INVALID_TWEET': 'Tweet non valido',
    'TWITTER.LOGIN_EXPIRED': 'Accesso scaduto',
    'TWITTER.USERNAME_TAKEN': 'Questo nome utente esiste già',
    'TWITTER.FAIL_TO_CREATE': 'Impossibile creare il tuo account',
    'TWITTER.FAIL_TO_TWEET': 'Impossibile pubblicare il tuo tweet',
    'TWITTER.REPLY': '{name} ha risposto al tuo tweet',
    'TWITTER.RETWEET': '{name} ha retwittato il tuo tweet',
    'TWITTER.LIKE': '{name} ha apprezzato il tuo tweet',
    'TWITTER.FOLLOW': '{name} ti sta seguendo',
    'WHATSAPP.NUMBER_OUT_GROUP': 'Questo numero non fa parte del gruppo',
    'WHATSAPP.NUMBER_IN_GROUP': 'Questo numero è già nel gruppo',
    'WHATSAPP.GROUP_NOT_FOUND': 'Gruppo non trovato',
    'WHATSAPP.GROUP_FULL': 'Il gruppo è pieno',
    'WHATSAPP.GROUP_NOT_OWNER': 'Non sei il proprietario del gruppo',
    'WHATSAPP.OWNER_LEAVE': 'Non puoi uscire dal gruppo se ne sei il proprietario',
    'TOR.USER_OFFLINE': 'Utente offline',
    'TOR.PAYMENT_NOTIFY': 'Hai ricevuto {value} da @{user}',
    'TOR.PAYMENT_RECEIPT': 'Hai inviato {value} a @{user}',
    'TOR.BLOCKED': 'Non puoi accedere a questa applicazione'
};
const pt = ps;
...
RegisterCommand('smartphone-setbase', (c, d) => {
    c == 0 && d.length && (pC(e => e.base = d[0]), console.log('Base alterada para: \'' + d[0] + '\' (Riavvia lo script per vedere le modifiche)'));
}), RegisterCommand('smartphone-beta', c => {
    if (c == 0) {
        pC(e => e.channel = 'beta'), console.log('Canale di aggiornamento modificato in beta (riavvia lo script per vedere le modifiche)');
    }
}), RegisterCommand('smartphone-stable', c => {
    if (c == 0) {
        pC(d => delete d.channel), console.log('Canale di aggiornamento modificato in stabile (riavvia lo script per vedere le modifiche)');
    }
});
if (pB.base === 'creative_v3') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (c, d) => {
    ...
    if (pB.base === 'creative_v4') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (d, e) => {
        ...
        } else pB.base === 'creative_network' && (pB.serverSideProp = true, pp.getUserSource = pp.Source, pp.getUserId = pp.Passaporto, pp.getInventoryItemAmount = pp.InventoryItemAmount, pp.hasPermission = pp.HasPermission, pp.request = pp.Request, pp.getBankMoney = async c => {
        ...
        return new Promise(async c => {
            while (true) {
                try {
                    ...
                    console.error('Impossibile caricare le tabelle dal database, se ciò accade spesso il connettore del database ha un problema'), await p2(1000);
                }
            }
...
pF.ready(async d => {
    ...
    if (f) {
        ...
        await j('smartphone_whatsapp_messages', f.whatsapp), await j('smartphone_tor_messages', f.tor), await j('smartphone_olx', f.olx), await j('smartphone_calls', f.calls), await j('smartphone_twitter_tweets', f.twitter);
    }
    ...
    if (d != null) {
        if (pB.item != false) {
            if (GetResourceState('pd-inventory') == 'started') {
                var e = await exports['pd-inventory'].getItemAmount(d, pB.item || 'cellulare');
            } else {
                var e = await pp.getInventoryItemAmount(d, pB.item || 'cellulare');
...
function pW() {
    pV < Date.now() && (console.error('Impossibile creare l\'elica del cellulare, questo problema è serio ma ignorabile [ENTITY DELETED]'), pV = Date.now() + 300000);
    return false;
}
...
onNet('backend:req', async (e, f, g) => {
    ...
    if (!pL[f]) {
        return console.error(f + ' non esiste nel backend');
    }
    ...
    try {
        while (true) {
            ...
            return await pF.fetchTables(), await Promise.resolve().then(function () {
                ...
                return s9;
            }), emit('smartphone:isReady'), console.log('[SMARTPHONE] Autorizzato!'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
        }
    } catch (x) {
...
function qf({
    ...
    if (typeof config.webhookURL === 'string' && config.webhookURL.startsWith('http')) {
        ...
        return pz.post(config.webhookURL, g).catch(() => {
            ...
            qe = Date.now() + 15000, console.error('Impossibile inviare un webhook a discord, questo non era un problema mobile, piuttosto un problema di rete che non sei in grado di risolvere, aspetta. (NON INVIARE QUESTO ERRORE SU DISCORD)');
        });
    }
...
pL.service_request = async (i, j, k, l) => {
    ...
    if (p) {
        ...
        if (r.length === 0) {
            ...
            if (p.dev) {
                ...
                for (let B of r) {
                    ...
                    const C = await pM(B);
                    if (!C) continue;
                    emitNet('chatMessage', C, 'CHIAMATA', [19, 197, 43], '(' + p.name + ')  Inviato da ^1' + o + '^0 [' + m + '], ' + k);
                    const D = {};
                    D.user_id = m, D.phone = n, D.name = o, emitNet('smartphone:service_request', C, D), pp.request(C, 'Rispondere alla chiamata di ' + o + '?', 30).then(E => {
                        if (E) {
                            if (s) emitNet('Notify', C, 'negado', 'Questa chiamata ha già ricevuto risposta');else {
                                s = true;
                                const H = {};
        ...
        const F = {
            'error': 'Questo servizio non esiste'
        };
        return F;
    }
};
...
pL.createPhoneCall = async (h, i, j = false, k = false) => {
    ...
    n = {
        'error': 'Stai già effettuando una chiamata'
    };
    ...
    if (f) {
        ...
        const l = {
            'error': 'Questo collegamento non esiste più'
        };
        return l;
    }
...
async function qy(d, e) {
    ...
    if (f) {
        ...
        if (pF.hasTable('summerz_bank')) {
            const l = await pF.firstColumn('summerz_bank', 'user_id', 'Passaporto'),
            ...
            console.error('Trasferimento offline non adattato, apri un ticket discord per richiedere l\'adattamento');
        }
    }
}
...
async function qA({
    ...
    if (p5('addBankStatement')) {
        ...
        if (pF.hasTable('smartbank_statements')) {
            ...
            if (!r) {
                console.error('Giocatore senza pix su smartbank: ' + h + ' [' + r + ']');
                return;
            } else {
                if (l) {
                    const u = {
    ...
    if (await pF.hasColumns('vrp_banco', 'title', 'amount', 'idtrans')) {
        const C = {};
        C.user_id = h, C.amount = j, C.idtrans = i ?? h, C.title = l ? 'Fattura' : k ? 'Multa' : 'Bonifico', await pc(o).insert(C);
    } else {
        if (pF.hasTable('wise_multas')) {
            let D = l ? 'Pagamento delle fatture' : k ? 'Pagamento della multa' : 'Bonifico';
            if (!m) j *= -1;
            return qA.toWise(h, j, D);
        } else {
            if (o) {
                const F = pu(j);
                let G = 'Effettuato un bonifico ' + F;
                if (l) G = 'Ha pagato una fattura ' + F;else {
                    if (k) {
                        G = 'Pagato ' + F + ' La multa';
                    } else {
                        if (m) {
                            G = 'Hai ricevuto un trasferimento da ' + F;
                        }
                    }
                }
                await pc(o).insert({
...
async function qC(d, e) {
    if (qB[d]) {
        const f = {
            'error': 'Attendi il tuo trasferimento precedente'
        };
        return f;
    }
...
pF.ready(async () => {
    ...
    for (let [h, i] of Object.entries(g)) {
        e.push({
            'table': h,
            'userColumn': await pF.firstColumn(h, 'Passaporto', 'id'),
            'column': i,
    ...
    for (let [k, l, m] of [['fines', 'text', 'price'], ['characters_fines', 'text', 'price'], ['fines', 'motivo', 'valor'], ['rb_multas', 'motivo', 'valor'], ['vrp_multas', 'motivo', 'valor']]) {
        e.push({
            ...
            async 'getAll'(n) {
                const p = await pF.firstColumn(this.table, 'id', 'Passaporto'),
                q = {};
                q.user_id = n;
                const r = {};
            ...
            async 'getOne'(n) {
                const o = await pF.firstColumn(this.table, 'id', 'Passaporto');
    ...
    if (pF.hasTable('smartbank_fines')) return qE = {
        ...
        'getAll'(o) {
            return this.select('id, importo come totale, causale come descrizione', o);
        },
    ...
    if (!qE) {
        console.error('Non è stato trovato alcun sistema di multa compatibile, apri un ticket su discord richiedendo l\'adattamento del tuo modello attuale');
        const t = {};
    ...
    if (p5('getTotalFines')) {
        ...
        if (qE) h = await qE.getSum(e);else console.error('Nessun sistema di multa trovato');
    }
    ...
    if (p5('getBankStatements')) {
        ...
        if (pF.hasTable('smartbank_statements')) {
            ...
            if (!k) {
                return console.error('Giocatore senza smartbank pix: ' + h + ' [' + k + ']'), [];
            }
            ...
            if (await pF.hasColumns('vrp_banco', 'title', 'amount', 'idtrans')) {
                ...
                if (pF.hasTable('fines')) {
                    ...
                    if (i) {
                        const v = {};
                        return v.user_id = h, pc(i).where(v).selectRaw('id', 'estrarre la descrizione dell\'AS', 'data AS created_at').orderBy('id', 'DESC').limit(100);
                    } else {
                        if (pF.hasTable('cactus_statements')) {
                            ...
                            for (let y of x) {
                                ...
                                y.description = (A ? 'Entrata' : 'Uscita') + ' di ' + pu(y.amount);
                            }
    ...
    if (p5('getFines')) {
        ...
        if (qE) {
            return qE.getAll(d);
        } else console.log('Nessun sistema di multa trovato');
    }
    ...
    return qC(e, async () => {
        if (p5('payFine')) {
            ...
            if (qE) {
                ...
                const r = {
                    'error': 'Nessun sistema di multa trovato'
                };
                return r;
            }
        }
...
pL.bank_pix = async (e, f, g) => {
    ...
    if (!Number.isInteger(h) || h <= 0) {
        ...
        if (g == j) {
            const n = {
                'error': 'Non puoi trasferire a te stesso'
            };
            return n;
        } else {
    ...
    if (v) {
        ...
        z = 'Vuoi accettare la fattura {reason} di {name} nella quantità di {value}'.format({
            'reason': o,
            'name': y,
            'value': pu(n)
        });
    ...
    if (!i) return j;
    const k = {
        'error': 'Questa fattura non è tua'
    };
    if (i.payer_id != h) return k;
    const l = {
        'error': 'Questa fattura è già pagata'
    };
    if (i.paid) return l;
    return qC(h, async () => {
...
pL.ig_accounts = async d => {
    ...
    if (t >= r) {
        const x = {
            'error': 'Hai raggiunto il numero massimo di account'
        };
        return x;
    } else {
        if (!Number.isInteger(q)) {
            const z = {
                'error': 'Impossibile recuperare il passaporto'
            };
            return z;
        } else {
            if (!n.match(qN.regex)) {
                const A = {
                    'error': 'Utente non valido, utilizzare lettere/numeri con un massimo di 24 caratteri'
                };
                return A;
            } else {
                if (!m || m.length > 32) {
    ...
    if (v) {
        const E = {
            'error': 'Questo nome utente esiste già'
        };
        return E;
    } else {
...
pL.ig_setFollow = async (f, g, h) => {
    ...
    if (!j) {
        ...
        if (h) {
            if (!qT[i + '-' + g]) {
                ...
                n.profileId = g;
                const o = await qV(n);
                qN.notify(o.id, j.id, '{name} ti ha seguito'.format({
    ...
    const n = {
        'error': 'Riaprire la domanda (comunicare al municipio)'
    };
    if (!m) return n;
    const o = await qO(j),
    p = {
        'error': 'Questa pubblicazione non è stata trovata.'
    };
    if (!o) return p;
    const q = {
        'profile_id': m.id,
    ...
    const v = await qV(u);
    qN.notify(v.id, m.id, '{name} ha commentato il tuo post'.format({
...
pL.ig_createPost = async (f, g, h) => {
    ...
    if (m && m.length) {
        ...
        qN.notifyAll(t, j.id, '{name} Ti ha menzionato in un post'.format({
            'name': j.name || j.username
        }));
    }
    
    const n = await qN.followers.where('profile_id', j.id).pluck('follower_id');
    qN.notifyAll(n, j.id, '{name} Ha postato una foto'.format({
        'name': j.name || j.username
    }));
    const o = {
        'TIPO': 'Publicação'
...
pL.ig_setLike = async (h, i, j) => {
    ...
    if (!l) return false;else {
        if (j) {
            if (!qW[k + '-' + i]) {
                ...
                const t = await qV(s),
                u = {};
                u.name = m || n, qN.notify(t.id, l, '{name} Ha messo like al tuo post'.format(u));
            }
    ...
    if (!qN.cache) {
        ...
        for (let j of e) {
            ...
            if (!l) {
                console.error('l\'utente instagram ' + j.profile_id + ' non trovato (è stato cancellato manualmente dal database)');
                continue;
            }
...
pL.ig_updateProfile = async (i, j) => {
    ...
    if (l && j) {
        ...
        if (n != l.username) {
            if (qX.get(k) > oT()) {
                const v = {
                    'error': 'Attendi un\'ora per modificare nuovamente il nome utente'
                };
                return v;
            }
...
pF.ready(async c => {
    ...
    if (u) return v;else {
        ...
        x = {
            'error': 'Impossibile creare il tuo account'
        };
        return !w && x;
    }
...
pL.twitter_profile = async (d, e) => {
    ...
    const q = {
        'error': 'Impossibile registrare il tuo tweet'
    };
    if (!p.id) return q;
    const r = { ...p,
        'likes': 0x0,
    ...
    w.content = v, qf(w);
    const x = {};
    return x.name = o.name, r0.notify(l, j, '{name} ha risposto al tuo tweet'.format(x)), s;
    ...
    if (g && !p0(d, 'retweet-' + e, 1)) {
        ...
        const h = {};
        return h.name = g.name, r0.notify(e, d, '{name} Ti ha retweettato'.format(h), [g.id, 'rt', e]), true;
    }
    ...
    if (h && !p0(e, 'like-' + f, 1)) {
        ...
        const j = {};
        return j.name = h.name, r0.notify(f, e, '{name} è piaciuto il tuo tweet'.format(j), [h.id, 'like', f]), true;
    }
    ...
    if (h && !p0(e, 'follow-' + f, 1)) {
        ...
        const j = {};
        return j.name = h.name, r0.notifyProfile(f, e, '{name} ti ha seguito'.format(j), [h.id, 'follow', f]), true;
    }
    ...
    if (i) {
        ...
        if (k || (await pK(h))) {
            ...
            const n = {
                'TIPO': 'Tweet cancellato'
            };
            n.ID = h, n['POST ID'] = g, n.POSTAGEM = j.content, n['POSTAGEM PROPRIA'] = k ? 'SI' : 'NO';
            const o = {
                'name': 'Twitter'
            };
            o.content = n, qf(o);
...
RegisterCommand('tor_find', async (c, [d, e = 100000]) => {
    if (c != 0) return;
    if (!d) console.log('Inserisci l\'ID Tor: tor_find 123456789');else {
        for (let g = 1; g <= e; g += 1) {
            if (rg.getId(g) == d) {
                return console.log('ID giocatore: ' + g);
            }
        }
    ...
    if (c != 0) return;
    if (!d) console.log('Inserisci l\'ID giocatore: tor_id 123456789');else {
        console.log('Il Tor id del giocatore ' + user_id + ' é ' + rg.getId(d));
    }
...
pL.tor_id = async c => {
    ...
    const d = await pp.getUserId(c);
    if (await pE(d, pB.tor_blocked)) return 'Non puoi accedere a questa applicazione';
    return false;
    ...
    return qC(i, async () => {
        ...
        if (s >= h) {
            ...
            const v = pu(h),
            w = {};
            w.value = v, w.user = j, oS(k, 'TOR_NOTIFY', 'Hai ricevuto {value} da @{user}'.format(w));
            const x = {};
            x.value = v, x.user = j, oS(f, 'TOR_NOTIFY', 'Hai inviato {value} a @{user}'.format(x));
            const y = {
                'TIPO': 'Pagamento'
            };
    ...
    if (!c.includes(g.table)) {
        ...
        for (let o of await rl.groups) {
            try {
                ...
                console.log('Il gruppo ' + o.id + ' è stato ignorato a causa di errori di formattazione');
            }
        }
...
pL.wpp_sendMessage = async (f, g, h, i = 'text', j = null) => {
    ...
    if (p0(f)) return {};else {
        if (h.length > 255) {
            const p = {
                'error': 'Messaggio troppo grande'
            };
            return p;
        } else {
            if (await rn(l, g)) {
                const r = {
                    'error': 'Non puoi inviare un messaggio a questo numero.'
                };
                return r;
            }
        }
...
pL.wpp_markAsRead = async (f, g) => {
    ...
    if (!j) {
        ...
        if (j.owner === i || j.members.includes(i)) {
            ...
            const p = {
                'error': 'Non sei il proprietario del gruppo'
            };
            return p;
        }
    }
    ...
    if (k.owner === j) {
        ...
        if (l >= 0) {
            ...
            const n = {
                'error': 'Questo numero non fa parte del gruppo'
            };
            return n;
        }
    } else {
        const q = {
            'error': 'Senza permesso'
        };
        return q;
    }
    ...
    if (!m) {
        ...
        if (m.owner === l) {
            if (m.members.includes(j)) {
                const r = {
                    'error': 'Questo numero è già nel gruppo'
                };
                return r;
            } else {
                if (m.members.length >= rm.MAX_MEMBERS - 1) {
                    const s = {
                        'error': 'Il gruppo è pieno'
                    };
                    return s;
                } else {
    ...
    if (!j) {
        ...
        if (j.owner === i) {
            const n = {
                'error': 'Non puoi lasciare il gruppo come proprietario'
            };
            return n;
        } else {
...
pF.ready(async c => {
    ...
    const q = {
        'error': 'Il numero massimo di immagini è 3'
    };
    ...
    if (k && (k.user_id == i || (await pK(i)))) {
        ...
        const m = {};
        m.ID = i, m.ANUNCIO = k.title, m['ID dell\'annuncio'] = k.id, m['TITOLARE DELL\'ANNUNCIO'] = k.user_id;
        const n = {
            'name': 'OLX'
        };
        n.content = m, qf(n);
...
function rw(c) {
    const d = c.target == 'All' ? ['Maschio', 'Femmina'] : [c.target];
...
async function rH(c, d) {
    if (!(await pE(c, pB.exclusive?.['weazel']?.['permission']))) {
        throw new p8('Senza permesso');
    } else {
        if (!d.author) {
            throw new p8('Inserisci il nome dell\'autore');
        } else {
            if (!d.title) {
                throw new p8('Inserisci il titolo della notizia');
            } else {
                if (!d.description) {
                    throw new p8('Inserisci la descrizione della notizia');
                } else {
                    if (!d.tag) {
                        throw new p8('Inserisci la categoria della notizia');
                    }
                }
            }
        }
...
rF.weazel_view = async (d, e) => {
    ...
    if (f) {
        ...
        if (g) {
            await rE().destroy(d), oS(-1, 'WEAZEL_DESTROY', d), qf({
                'name': 'Weazel - Notizie cancellate',
                'avatar': 'weazel',
                'content': {
                    'PASSAPORTE': e,
                    'NOTICIA': g.title,
    ...
    k.id = f, await rE().where(k).update(i), qf({
        'name': 'Weazel - Notizie aggiornate',
        'avatar': 'weazel',
        'content': {
            'PASSAPORTE': h,
...
rP.casino_double = () => {
    ...
    if (!j || j <= 0) {
        ...
        if (rY.rolling) {
            const p = {
                'error': 'Aspetta che la roulette finisca il gioco in corso'
            };
            return p;
        } else {
            if (rY.getBets(k) + j > rM) return {
                'error': 'Il limite di puntata è ' + pu(rM)
            };
        }
    }
    ...
    if (m < j) {
        ...
        if (j > rM) {
            const v = {};
            return v.error = 'Il limite di puntata è ' + rM, v;
        }
    }
...
global.on('playerDropped', () => delete s0[source]), rP.casino_mine_start = async (j, k, l) => {
    ...
    if (!Number.isSafeInteger(m)) {
        ...
        if (m in s0) {
            const u = {
                'error': 'Sei già in un gioco'
            };
            return u;
        } else {
            if (!(k = parseInt(k)) || k <= 0) {
                const v = {
                    'error': 'Importo della scommessa non valido'
                };
                return v;
            } else {
                if (!(l = parseInt(l)) || l < 1 || l > 24) {
                    const x = {
                        'error': 'Quantità di mine non valida'
                    };
                    return x;
                } else {
                    if (k > rM) {
                        return {
                            'error': 'Il limite per scommessa è ' + pu(rM)
                        };
                    }
                }
            }
    ...
    if (j.reward == 0) {
        const s = {
            'error': 'Non puoi abbandonare il gioco ora.'
        };
        return s;
    }
    ...
    if (j.revealed[h]) {
        const r = {
            'error': 'Hai già rivelato questa miniera'
        };
        return r;
    }
...
rP.casino_crash = c => ({
    ...
    if (!k || k <= 0) {
        ...
        if (s7.status === 'crashing') {
            const s = {
                'error': 'Attendi che il gioco in corso finisca'
            };
            return s;
        } else {
    ...
    if (o < k) {
        ...
        if (l && typeof l !== 'number') {
            const u = {
                'error': 'Importo di prelievo automatico non valido'
            };
            return u;
        }
    }
    ...
    if (!k.length) {
        const p = {
            'error': 'Non hai piazzato alcuna scommessa'
        };
        return p;
    } else {
        if (!l.length) {
            const q = {
                'error': 'Il tuo premio è già stato riscattato'
            };
            return q;
        } else {
            if (s7.status !== 'crashing') {
                const r = {
                    'error': 'Il gioco non è ancora iniziato'
                };
                return r;
            }
        }
