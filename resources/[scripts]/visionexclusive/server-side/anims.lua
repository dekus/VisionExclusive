...
CreateCommand = RegisterCommand
-----------------------------------------------------------------------------------------------------------------------------------------
-- /e
-----------------------------------------------------------------------------------------------------------------------------------------
CreateCommand('e',function(source,args,rawCommand)
	...
	if nplayer and not vRPclient.isInVehicle(nplayer) and not vRPclient.isHandcuffed(nplayer) and vRPclient.getHealth(nplayer) > 101 then
		if args[1] == "beijar" then
			if vRP.request(nplayer,"Deseja beijar <b>"..identity.name.."</b> ?",5) then
				...
				TriggerClientEvent("Notify",source,'negado',"Negado","A pessoa negou o beijo.")
			end
		elseif args[1] == "abracar" then
			if vRP.request(nplayer,"Deseja abraçar <b>"..identity.name.."</b> ?",5) then
		...
		elseif args[1] == "dancar257" then
			if vRP.request(nplayer,"Deseja dançar com <b>"..identity.name.."</b> ?",5) then
				...
				TriggerClientEvent("Notify",source,'negado',"Negado","A pessoa negou a dança.")
			end
		...
		elseif args[1] == "casal" then
			if vRP.request(nplayer,"Deseja casal com <b>"..identity.name.."</b> ?",5) then
				...
				TriggerClientEvent("Notify",source,'negado',"Negado","A pessoa negou o casal.")
			end
		end		
	end
...
-- /e2
-----------------------------------------------------------------------------------------------------------------------------------------
CreateCommand('e2', function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
